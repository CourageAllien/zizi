...
// Lead Gen email types
interface LeadGenBookingEmailProps {
  to: string;
  name: string;
  offer: string;
  idealClient: string;
...
/**
 * Send Lead Gen booking confirmation email
 */
export async function sendLeadGenBookingConfirmationEmail({
  to,
  name,
  offer,
  idealClient,
  ...
  time,
  bookingId,
}: LeadGenBookingEmailProps): Promise<{ success: boolean; error?: string }> {
  const firstName = name.split(" ")[0];
  const formattedDate = formatDate(date);
  const formattedTime = formatTime(time);

  try {
    console.log("Resend: Sending lead gen booking confirmation to:", to, "from:", FROM_EMAIL);
    const result = await resend.emails.send({
      from: FROM_EMAIL,
      to,
...
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #0a0a0a;">
  <div style="max-width: 600px; margin: 0 auto; background: linear-gradient(135deg, #0a0a0a 0%, #0d1a0d 100%); padding: 40px 30px;">
    ...
    <div style="text-align: center; margin-bottom: 40px;">
      <h1 style="color: #22c55e; font-size: 32px; margin: 0; font-weight: bold;">ZiziCo</h1>
      <p style="color: #9ca3af; margin: 5px 0 0;">Lead Gen Tools for High-Ticket Sellers</p>
    </div>
    
    <!-- Confirmation Badge -->
    <div style="text-align: center; margin-bottom: 30px;">
      <div style="display: inline-block; background: linear-gradient(135deg, #22c55e, #16a34a); padding: 15px 30px; border-radius: 50px;">
        <span style="color: white; font-size: 18px; font-weight: 600;">✓ Strategy Call Confirmed!</span>
      </div>
    </div>
    ...
    <p style="color: #9ca3af; font-size: 16px; line-height: 1.6; margin-bottom: 30px;">
      Your 60-minute strategy call is locked in. We're excited to learn about your high-ticket offer and discuss how we can build lead gen tools that attract your ideal clients.
    </p>
    
    <!-- Meeting Details Card -->
    <div style="background: #141414; border: 1px solid #262626; border-radius: 16px; padding: 25px; margin-bottom: 30px;">
      <h2 style="color: #22c55e; font-size: 20px; margin: 0 0 20px; font-weight: 600;">📅 Meeting Details</h2>
      
      <div style="margin-bottom: 15px;">
        <div style="color: #737373; font-size: 14px; margin-bottom: 5px;">Date</div>
        <div style="color: #ffffff; font-size: 18px; font-weight: 600;">${formattedDate}</div>
      </div>
      
      <div style="margin-bottom: 15px;">
        <div style="color: #737373; font-size: 14px; margin-bottom: 5px;">Time</div>
        <div style="color: #ffffff; font-size: 18px; font-weight: 600;">${formattedTime}</div>
      </div>
      
      <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #262626;">
        <a href="${ZIZI_ZOOM_MEETING.link}" style="display: inline-block; background: linear-gradient(135deg, #22c55e, #16a34a); color: white; text-decoration: none; padding: 12px 24px; border-radius: 8px; font-weight: 600; text-align: center;">
          Join Zoom Meeting →
        </a>
      </div>
    </div>
    ...
    <!-- What to Expect -->
    <div style="background: #141414; border: 1px solid #262626; border-radius: 16px; padding: 25px; margin-bottom: 30px;">
      <h2 style="color: #22c55e; font-size: 20px; margin: 0 0 15px; font-weight: 600;">What We'll Cover</h2>
      <ul style="color: #9ca3af; font-size: 15px; line-height: 1.8; margin: 0; padding-left: 20px;">
        <li>Your high-ticket offer and ideal client</li>
        <li>What lead gen tools would work best for you</li>
        <li>How the $549/month partnership works</li>
        <li>Next steps to get your first tool built</li>
      </ul>
    </div>
    ...
    <div style="text-align: center; margin-top: 40px; padding-top: 30px; border-top: 1px solid #262626;">
      ...
      <p style="color: #737373; font-size: 12px; margin: 10px 0 0;">
        Questions? Reply to this email or reach out at hello@zizi.so
      </p>
    </div>
    ...
    console.log("Resend: Lead gen booking confirmation result:", JSON.stringify(result));
    ...
    console.error("Error sending lead gen booking confirmation email:", error);
    return { success: false, error: String(error) };
  }
}

/**
 * Send Lead Gen booking admin notification
 */
export async function sendLeadGenBookingAdminNotification({
  to,
  name,
  offer,
  idealClient,
  ...
  try {
    console.log("Resend: Sending lead gen booking admin notification to:", ADMIN_EMAIL);
    const result = await resend.emails.send({
      from: FROM_EMAIL,
      to: ADMIN_EMAIL,
      subject: `📅 Lead Gen Strategy Call Booked: ${name}`,
      html: `
<!DOCTYPE html>
<html>
<head>
...
<body style="margin: 0; padding: 20px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f3f4f6;">
  <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    ...
    <!-- Header -->
    <div style="background: linear-gradient(135deg, #22c55e, #16a34a); padding: 25px; text-align: center;">
      <h1 style="color: white; margin: 0; font-size: 24px;">New Lead Gen Strategy Call! 🎉</h1>
    </div>
    ...
    <div style="padding: 30px;">
      ...
      <div style="background: #f9fafb; border-left: 4px solid #22c55e; padding: 15px; margin-bottom: 25px; border-radius: 0 8px 8px 0;">
        <p style="color: #374151; margin: 0; line-height: 1.6;">${offer}</p>
      </div>
      ...
      <div style="background: #f9fafb; border-left: 4px solid #22c55e; padding: 15px; margin-bottom: 25px; border-radius: 0 8px 8px 0;">
        <p style="color: #374151; margin: 0; line-height: 1.6;">${idealClient}</p>
      </div>
      
      ${currentLeadGen ? `
      <h3 style="color: #6b7280; margin: 0 0 10px; font-size: 14px;">Current Lead Gen:</h3>
      <div style="background: #f9fafb; padding: 15px; margin-bottom: 25px; border-radius: 8px;">
        <p style="color: #374151; margin: 0; line-height: 1.6;">${currentLeadGen}</p>
      </div>
      ` : ""}
    ...
    console.log("Resend: Lead gen booking admin notification result:", JSON.stringify(result));
    ...
    console.error("Error sending lead gen booking admin notification:", error);
    return { success: false, error: String(error) };
  }
}
